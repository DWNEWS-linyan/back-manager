...
//						$("#son").html("成功公里了");
	...
	return {
		...
		getObjectURL:function(fisle){//建立一個可存取到該file的url
			var urlFil = null ;
			if (window.createObjectURL!=undefined) { // basic
				urlFil = window.createObjectURL(fisle) ;
			} else if (window.URL!=undefined) { // mozilla(firefox)
				urlFil = window.URL.createObjectURL(fisle) ;
			} else if (window.webkitURL!=undefined) { // webkit or chrome
				urlFil = window.webkitURL.createObjectURL(fisle) ;
			}
			return urlFil ;
		}
	}
